...
def _reindex_upload_dir(upload_dir: Path):
    build_index(upload_dir)

@router.post("/upload", response_model=UploadResponse, dependencies=[Depends(verify_api_key)])
async def upload_and_index(background: BackgroundTasks, files: List[UploadFile] = File(...)):
    if not files:
        raise HTTPException(status_code=400, detail="No file is provided")
    ...
    # enqueue indexing so the response returns immediately
    background.add_task(_reindex_upload_dir, upload_dir)

    return UploadResponse(
        saved_files=saved_paths,
        added=0,
        total_in_collection=None,
        message="Uploaded (indexing started in background).",
    )
