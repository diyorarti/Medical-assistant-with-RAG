...
from rag.api.models.io import QueryRequest, QueryResponse
...
def query_rag(req: QueryRequest):
    """
    Ask a question against the vector store; returns answer + citations.
    """
    # Provider checks for required tokens
    if req.provider == "grok" and not settings.GROK_API_KEY:
        raise HTTPException(status_code=400, detail="Grok requested, but GROK_API_KEY is not set.")
    if req.provider == "hf" and not settings.HF_TOKEN:
        raise HTTPException(status_code=400, detail="HF requested, but HF_TOKEN is not set.")
